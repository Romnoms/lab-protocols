...
Modified to show ALL sections by injecting JavaScript before PDF generation
"""
import asyncio
...
async def convert_to_pdf(html_file):
    """Convert a single HTML file to PDF with all sections visible"""
    ...
    try:
        async with async_playwright() as p:
            ...
            await page.wait_for_timeout(1000)  # Give Alpine.js time to initialize

            # Inject JavaScript to make ALL sections visible
            await page.evaluate("""
                () => {
                    // Remove all x-show directives to show all content
                    document.querySelectorAll('[x-show]').forEach(el => {
                        el.removeAttribute('x-show');
                        el.style.display = 'block';
                    });

                    // Hide navigation tabs for cleaner PDF
                    const nav = document.querySelector('nav');
                    if (nav) nav.style.display = 'none';

                    // Remove the "back to protocols" button for cleaner print
...
async def main():
    ...
    print("Starting PDF conversion with ALL sections visible...")
